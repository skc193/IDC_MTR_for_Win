...
def generate_report(db_path, target, report_path, hours=24):
    ...
    # ── 警報表格 ──
    if not alert_rows:
        alert_html = "<p style='color:#484f58;padding:12px'>最近 {} 小時無警報</p>".format(hours)
    else:
        rows = ""
        for r in alert_rows:
...
header{{background:linear-gradient(135deg,#161b22,#21262d);padding:18px 28px;border-bottom:1px solid #30363d}}
...
.kpi .v{{font-size:1.7rem;font-weight:bold;margin:5px 0}}
.kpi .l{{color:#8b949e;font-size:.78rem}}
.card{{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:18px;margin-bottom:16px}}
.card h2{{font-size:.9rem;color:#8b949e;margin-bottom:12px}}
canvas{{max-height:230px}}
table{{width:100%;border-collapse:collapse;font-size:.83rem}}
th,td{{padding:8px 11px;text-align:left;border-bottom:1px solid #21262d}}
th{{color:#8b949e;font-weight:500}}
tr:hover{{background:#1c2128}}
td.ip{{font-family:monospace;color:#79c0ff}}
...
.sec{{font-size:.95rem;color:#58a6ff;margin:6px 0 12px;border-left:3px solid #58a6ff;padding-left:10px}}
.footer{{text-align:center;color:#484f58;font-size:.78rem;padding:16px}}
.os-badge{{display:inline-block;background:#21262d;border:1px solid #58a6ff;border-radius:6px;padding:2px 10px;font-size:.78rem;color:#58a6ff;margin-left:10px}}
.status-pill{{display:inline-block;padding:4px 14px;border-radius:14px;font-size:1rem;font-weight:bold}}
</style>
</head>
<body>
<header>
  <h1>🛰️ MTR 全路由節點監測 <span class="os-badge">Windows 版</span></h1>
  <p>目標：{target} | 最近 {hours} 小時 | 掃描 {scan_count} 次 | 工具：tracert + ping | 更新：{now_str}</p>
</header>
<div class="container">
  <div class="kpi-grid">
    ...
    <div class="kpi"><div class="l">最終節點</div><div class="v" style="font-size:1rem;color:#79c0ff">{final_label}</div></div>
  ...
  <div class="card">
    <div class="sec">📊 各跳點 {hours} 小時統計</div>
    {history_html}
  </div>
...
const labels=rawLabels.map(function(s){{return s.slice(11,16)}});
const baseOpt={{responsive:true,animation:false,
  plugins:{{legend:{{labels:{{color:'#8b949e'}}}}}},
  scales:{{x:{{ticks:{{color:'#484f58',maxTicksLimit:12}},grid:{{color:'#21262d'}}}},
...
new Chart(document.getElementById('lossChart'),{{type:'bar',
  ...
  options:lossOpt}});
        ...
        target=html_escape(target), hours=hours, scan_count=scan_count, now_str=now_str,
